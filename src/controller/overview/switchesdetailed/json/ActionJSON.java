...
public class ActionJSON {
	...
	public static List<Action> getActions(JSONObject obj)
			throws JSONException,IOException{
		List<Action> actions = new ArrayList<Action>();
		if(!obj.has("actions")){
			return actions;
		...
		for (String string : list) {
			...
			if(s[0].equals("output"))
			...
			else if (s[0].equals("set_eth_dst_"))
				actions.add(new Action(s[0], s[1]));
				...
				//String queue[] = s[1].split("q");
				//String value = queue[0] + ":" + queue[1];
				//actions.add(new Action(s[0], value));
				actions.add(new Action(s[0], s[1]));
			}
			else
